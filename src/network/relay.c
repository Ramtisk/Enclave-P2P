...
int relay_process_message(relay_server_t *server, int client_index, message_t *msg)
{
    ...
    case MSG_FILE_LIST:
    {
        // Client is requesting file list from group
        ...
        // Forward request to all other group members
        // They will respond with MSG_FILE_ANNOUNCE for their files
        for (int i = 0; i < group->member_count; i++)
        {
            if (strcmp(group->members[i].client_id, msg->header.sender_id) != 0)
            {
    ...
    case MSG_NAT_DISCOVER:
        return relay_handle_nat_discover(server, client_index, msg);

    case MSG_NAT_PUNCH_REQ:
        return relay_handle_nat_punch_req(server, client_index, msg);

    case MSG_NAT_PUNCH_RESULT:
        return relay_handle_nat_punch_result(server, client_index, msg);

    case MSG_NAT_RELAY_DATA:
        return relay_handle_nat_relay_data(server, client_index, msg);
...
int relay_handle_nat_discover(relay_server_t* server, int client_index, message_t* msg) {
    ...
    // Respond with the client's public endpoint as we see it
    message_t response;
    memset(&response, 0, sizeof(response));
    ...
    // Simple NAT detection: compare with what they told us
    ...
    if (msg->header.payload_length >= sizeof(payload_punch_request_t)) {
        if (strcmp(req->local_ip, client->ip) == 0) {
            ...
            info->nat_type = NAT_TYPE_RESTRICTED_CONE; // Default assumption
        }
    } else {
...
int relay_handle_nat_punch_req(relay_server_t* server, int client_index, message_t* msg) {
    ...
    // Send punch instructions to BOTH peers
    
    // Instruction for requester (you are initiator)
    message_t instr_a;
    ...
    // Instruction for target (you are responder)
    message_t instr_b;
...
int relay_handle_nat_punch_result(relay_server_t* server, int client_index, message_t* msg) {
    ...
    LOG_INFO("NAT: Punch result from %s → %s: %s", 
             ...
             result->success ? "SUCCESS (direct)" : "FAILED (need relay)");
